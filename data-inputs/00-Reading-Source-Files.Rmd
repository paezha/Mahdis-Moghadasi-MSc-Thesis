...
```{r}
gss_e_2015 <- read.csv(paste0(here(), "/data-inputs/source-files/2015_Time use (GSS)/gss-e.csv"))
gss_e_2010 <- read.csv(paste0(here(), "/data-inputs/source-files/2010_Time use (GSS)/gss-12M0018-E-2010-c-24-tus-ef_F1.csv"))
gss_e_2005 <- read.csv(paste0(here(), "/data-inputs/source-files/2005_Time use (GSS)/gss-12M0019-E-2005-c-19-e_F1.csv"))
gss_e_1998 <- read.csv(paste0(here(), "/data-inputs/source-files/1998_Time use (GSS)/gss-12M0012-E-1998-c-12e_F1.csv"))
gss_e_1992 <- read.csv(paste0(here(), "/data-inputs/source-files/1992_Time use (GSS)/gss-12M0007-E-1992-c-7-ep_F1.csv"))
gss_e_1986 <- read.csv(paste0(here(), "/data-inputs/source-files/1986_Time use (GSS)/gss-12M0002-E-1986-c-2-ep_F1.csv"))
```
...
# Creating dataset of VALIDATION OF Impedance function
...
save(walking_data,
     file = paste0(here(), "/data/walking_data.Rda"),
     compress = "bzip2",
     version = 2,
     ascii = FALSE)
save(cycling_data,
     file = paste0(here(), "/data/cycling_data.Rda"),
     compress = "bzip2",
     version = 2,
     ascii = FALSE)
```
...
To conduct a descriptive analysis of walking and cycling trips, an integrated dataset named "trip" has been constructed. This dataset encompasses comprehensive data pertaining to both walking and cycling activities. 
...
```{r}
save(trip,
     file = paste0(here(), "/data/trip.Rda"),
     compress = "bzip2",
     version = 2,
     ascii = FALSE)
```

# Creating integrated dataset for providing impedance function graphs 

Two distinct datasets were constructed to analyze mobility patterns: walking_data_f and cycling_data_f. Each dataset was meticulously compiled to contain different modes of transportation, such as walking and cycling. A crucial step involved the integration of impedance functions into both datasets to further enhance our analysis. This was accomplished by creating a new column in each dataset, aptly named 'Impedance', which aggregates the various impedance functions corresponding to a range of destinations. These functions are designed to quantify individuals' resistance or difficulty when traveling to different locations. The inclusion of this column allows for a more nuanced understanding of how destination-specific factors influence walking and cycling behaviors. 
...
```{r}
save(walking_data_f,
     file = paste0(here(), "/data/walking_data_f.Rda"),
     compress = "bzip2",
     version = 2,
     ascii = FALSE)

save(cycling_data_f,
     file = paste0(here(), "/data/cycling_data_f.Rda"),
     compress = "bzip2",
     version = 2,
     ascii = FALSE)
```
